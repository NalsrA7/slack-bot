...
def engagement_counter():
    ...
    header = ['timestamp', 'emoji_count', 'reply_count']
    with open('engagement_count.csv', 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(header)
        for key, value in emojis.items():
            writer.writerow([key, str(value), (str(replies[key]) if key in replies else "0")])
